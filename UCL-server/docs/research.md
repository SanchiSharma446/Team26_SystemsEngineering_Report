...
| Weather and market price integration | One, this is something that farmers want. Two, real-time external data meaningfully improves the relevance of advice.  |
...
### 4.1 Frontend - React 19 with Vite

Our solution has a very modular UI, and to achieve this we chose React, which our team has familiarity with. Vite was used as the build tool for its fast module hot-reload during development. The main alternative considered was Next.js, which builds on React but adds server-side rendering. Since Cresco's backend is handled entirely by FastAPI and there was no need for server-rendered pages, the added complexity of Next.js was unnecessary, making React with Vite the leaner choice.
...
Cresco initially used SQLite as its database during early development. SQLite required no server setup, was easy to configure locally, and was sufficient for single-user testing. However, we were advised by our professors to migrate to PostgreSQL, as SQLite's limitations would become apparent as the project grew to handle concurrent users and more complex relational data.

### 4.6 LLM Provider - Configurable, defaulting to Azure OpenAI

Rather than hardcoding a single LLM provider, we built our tool to be configurable via environment variables, defaulting to Azure OpenAI. This decision was made early on, based on client feedback. Supporting multiple providers via `MODEL_PROVIDER` and `MODEL_NAME` variables also means Cresco can be switched to a local model via Ollama for offline or cost-sensitive deployments, which was important to the project's accessibility goals.
