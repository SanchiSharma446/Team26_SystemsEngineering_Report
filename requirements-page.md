...
The UML use case diagram uses standard notation: stick figures for actors, a system boundary rectangle labelled "Cresco", ovals for use cases, and dashed arrows with `<<include>>`/`<<extend>>` stereotypes.

**Actors:** Farmer (primary, left) — registered user with access to all features. External Services (secondary, right) — OpenWeatherMap, Nominatim, Copernicus Sentinel Hub, and Tavily Search APIs.

**Relationships:**
- UC5 (Send Chat Message) --`<<include>>`--> UC10 (Retrieve Agricultural Info)
- UC5 --`<<extend>>`--> UC11 (Search Internet) [condition: enabled via UC9]
- UC5 --`<<extend>>`--> UC12 (Generate Task List)
- UC5 --`<<extend>>`--> UC13 (Generate Inline Chart)
- UC14 (Upload Document) --`<<include>>`--> UC17 (Index Document into ChromaDB)
- UC18 (Set Farm Location) --`<<include>>`--> UC21 (Save Farm Data)
- UC18 --`<<include>>`--> UC22 (Reverse Geocode Location)
- UC18 --`<<extend>>`--> UC23 (Search Location by Address)
- UC24 (Upload Drone Images) --`<<include>>`--> UC25 (Select Vegetation Index Type)
- UC11, UC22, UC23 connect to External Services (Tavily, Nominatim)
- UC20 (Fetch Weather) connects to External Services (OpenWeatherMap)
- UC30 (Fetch Satellite NDVI) connects to External Services (Copernicus); depends on UC18
